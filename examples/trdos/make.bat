SET Z80_FLAGS=--allow-undocumented-instructions --no-std-crt0 --reserve-regs-iy --max-allocs-per-node 20000
...
SET LINKER_FLAGS=--code-loc 0x8010 --data-loc 0xA000 -l %LIBRARY_PATH%
SET BUILD_DIR=build
SET EXECUTABLE_NAME=trdos
...
"../../../pcsoft/trdtool/trdtool.exe" # %EXECUTABLE_NAME%.trd
@IF %ERRORLEVEL% EQU 1 GOTO ERR

"../../../pcsoft/trdtool/trdtool.exe" + %EXECUTABLE_NAME%.trd %EXECUTABLE_NAME%.tap
@IF %ERRORLEVEL% EQU 1 GOTO ERR

"../../../pcsoft/ZEsarUX_windows-13.0/zesarux.exe" --machine 128k --tape %EXECUTABLE_NAME%.tap
GOTO END
